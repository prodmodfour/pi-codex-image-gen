...
* `src/save/imageSave.ts` resolves `none`, `project`, `global`, and `custom` save modes, sanitizes session/image ids used in path parts, and writes generated bytes via a temporary file plus rename.
* `src/output/formatToolResult.ts` creates the Pi result with text summary, inline image content, MIME mapping, and sanitized details metadata.
* `src/pi/registerCodexImageGenTool.ts` registers the Pi tool, optional `/codex-image-gen` help command, and the execution pipeline that connects config, validation, Pi model-registry auth, Codex generation, saving, and result formatting.
...
Save/result formatting and Pi registration/execution wiring are covered by unit tests and use only fake image bytes under temporary directories. The backend communication layer is covered by fake tests only. It does not perform live Codex calls during the default quality gate; live validation remains reserved for the dedicated live-validation ticket.
