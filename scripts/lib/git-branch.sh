...
git_branch_current() {
  ...
  if [[ -n "$short_head" ]]; then printf 'DETACHED@%s\n' "$short_head"; else printf 'unknown\n'; fi
}

git_branch_validate_name() {
  local branch="$1"
  local label="${2:-branch}"
  if [[ -z "$branch" ]]; then pp_error "$label must not be empty."; return 2; fi
  if [[ "$branch" == -* ]]; then pp_error "$label must not start with '-': $branch"; return 2; fi
  if [[ "$branch" =~ [[:space:]] ]]; then pp_error "$label must not contain whitespace: $branch"; return 2; fi
...
git_branch_remote_matches() {
  local branch="$1"
  git for-each-ref --format='%(refname:short)' "refs/remotes/*/$branch" | grep -Ev '(^|/)HEAD$' || true
}
...
git_branch_select() {
  ...
  if [[ "$current_branch" == "$branch" ]]; then pp_success "Already on branch $branch."; return 0; fi
...
git_branch_prepare() {
  ...
  if [[ -n "$select_branch" ]]; then git_branch_select "$select_branch"; elif [[ -n "$create_branch" ]]; then git_branch_create "$create_branch" "$start_point"; fi
}
...
git_branch_push_current() {
  ...
  local upstream_ref
  current_branch="$(git branch --show-current 2>/dev/null || true)"
  if [[ -z "$current_branch" ]]; then pp_cmd "git push"; git push; return $?; fi
  upstream_ref="$(git rev-parse --abbrev-ref --symbolic-full-name '@{u}' 2>/dev/null || true)"
  if [[ -n "$upstream_ref" ]]; then pp_cmd "git push"; git push; return $?; fi
